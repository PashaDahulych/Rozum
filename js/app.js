(() => {
    ...
    function initSliders() {
        if (document.querySelector(".swiper")) new core(".swiper", {
            ...
            speed: 800,
            navigation: {
                prevEl: ".catalog__swiper-arrow-left.swiper-button-prev",
                nextEl: ".catalog__swiper-arrow-right.swiper-button-next"
            },
            breakpoints: {
                319: {
                    slidesPerView: 1,
    ...
    da.init();
    const userAuthorized = document.querySelector("[data-show-modal]");
    const infoUser = document.querySelector("[data-modal]");
    function showInfoUser() {
        infoUser.classList.add("_open-modal");
    }
    function hideInfoUser() {
        infoUser.classList.remove("_open-modal");
    }
    userAuthorized.addEventListener("click", (function(event) {
        if (event.target.closest("[data-show-modal]")) if (infoUser.classList.contains("_open-modal")) hideInfoUser(); else showInfoUser(); else hideInfoUser();
    }));
    document.addEventListener("click", (function(event) {
        if (!event.target.closest("[data-show-modal]") && !event.target.closest("[data-modal]")) hideInfoUser();
    }));
    window["FLS"] = true;
    isWebp();
    spollers();
